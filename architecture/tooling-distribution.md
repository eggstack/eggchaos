...
| `scripts/qualify_fuzz.sh` | Runs `plan_json`, `native_config`, `native_control_json`, `fault_evidence_json`, `policy_transitions`, and `toxiproxy_attributes` with `cargo fuzz --sanitizer none`, each for `${EGGCHAOS_FUZZ_RUNS:-10000}` runs. | Bounded parser/state-machine soak before release. Release workflow pins 10,000 runs per target. |
