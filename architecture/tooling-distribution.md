...
Pre-release `0.1.0`. All milestones M000–M015 plus M008 are closed;
tagging, crates.io publication, and GitHub release creation remain
...
| `scripts/qualify_fuzz.sh` | `(cd fuzz && cargo fuzz run plan_json --sanitizer none -- -runs="$runs")` where `runs="${EGGCHAOS_FUZZ_RUNS:-10000}"`; prints a `{"fuzz":"pass",...}` marker. | Bounded soak before release. Release workflow pins `EGGCHAOS_FUZZ_RUNS=10000`. |
| `scripts/qualify_toxiproxy_v2_12.sh` | `cargo test -p eggchaos-toxiproxy --all-features` (translation suite), then — only if a `TOXIPROXY_SERVER` oracle binary or `toxiproxy-server` on `PATH` reports `*2.12.0*` — the `--test differential` corpus, grepping for `"failed":0`. Without a pinned oracle it prints `{"translation":"pass","oracle":"unavailable","differential":"incomplete"}` and exits 0; with a wrong-version oracle it reports `differential:incomplete`. Non-pinned or missing oracle is recorded as incomplete evidence, never as proof. | Toxiproxy parity work; release differential gate. Invoke as `TOXIPROXY_SERVER=/path/to/pinned/v2.12.0 ./scripts/qualify_toxiproxy_v2_12.sh` (`docs/toxiproxy.md`, `plans/reference/toxiproxy-parity.md`). |
| `scripts/release-smoke.sh` | Full pre-publish gate: fmt + clippy (`-D warnings`) + workspace tests + doc + `cargo build --workspace --release` + `cargo audit --deny warnings` + `cargo deny check advisories licenses bans sources` + `cargo package -p eggchaos-core --allow-dirty` + `cargo package --list --allow-dirty` for all five crates + `cargo build --release --locked --package eggchaos-cli` + `./scripts/release-artifact-smoke.sh` + an embedded Python `cargo metadata --locked` order-publishability proof asserting every intra-workspace path dependency requires exactly `^{workspace_version}` from the registry, documenting order `core -> eggfetch -> server/toxiproxy/cli`. | Before any tag; first job step of the release `qualify` lane. |
| `scripts/release-artifact-smoke.sh` | Boots `target/release/eggchaos serve --config qualification/release/eggchaos.toml` (overridable as `$1`/`$2`), waits up to ~5 s for the `eggchaos listening; admin=` log line, then asserts: `/v1/health` contains `"running":true`; `--json proxy list` contains `"name":"smoke"`; `--json reset` contains `"reset":true`; `--json version` contains `"version":"0.1.0"`. Prints `{"artifact_smoke":"pass"}`; cleans up the child process and log on exit via trap. | Standalone after a release build; also called at the end of `release-smoke.sh` and as the last step of the release `qualify` job. |
...
smokes), `qualification/performance/` (snapshots), `fuzz/` (`plan_json`
target), `benchmarks/` (relay comparison harness).
...
10. Tag / crates.io publish (order `core -> eggfetch ->
    server/toxiproxy/cli` per `release-smoke.sh` order-proof) / GitHub
