...
| `scripts/benchmark_datagram.sh` | Runs the direct UDP echo/fixed-target benchmark in release mode, saves raw candidate/host/sample JSON when requested, then checks the measured no-fault throughput and p95 latency ratios. | Datagram performance qualification; the first M023 budget is ≥45% of same-session direct datagrams/s and ≤2.5× direct p95 latency. |
