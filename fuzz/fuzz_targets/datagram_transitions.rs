...
fuzz_target!(|data: &[u8]| {
    ...
    let Ok(plan) = serde_json::from_value::<DatagramPlan>(value) else {
        return;
    };
    let Ok(plan) = DatagramPlan::new(plan.faults().to_vec()) else {
        return;
    };
    ...
    let mut engine =
        DatagramDirectionEngine::new(limits, "fuzz", 1, Direction::Upstream, RngVersion::V1)
            .unwrap();
    let policy = eggchaos_core::DatagramLivePolicy::new(plan, 17)
        .unwrap()
        .snapshot();
    ...
    while let Some(deadline) = engine.next_deadline() {
        engine.take_ready(deadline);
    }
    let evidence = engine.evidence();
    assert_eq!(evidence.queued_datagrams, 0);
    assert_eq!(evidence.queued_bytes, 0);
    assert_eq!(
        evidence.emitted_datagrams
            + evidence.configured_loss
            + evidence.queue_overflow
            + evidence.oversize_datagrams,
        evidence.admitted_datagrams + evidence.duplicated_copies
    );
});
