...
Last reconciled: 2026-09-26 (M042–M045 performance tranche; M042 closed at the M042 baseline candidate; M043 and M044 now ready for the proven-material targets classified in M042 closure; M045 still blocked on both)
...
| M043 | `043-stream-hot-path-allocation-policy-timer-and-evidence-optimization.md` | **ready** | M042 (closed) | Stream hot-path optimization, evidence-gated by M042 target matrix. M042 classifies WP7 (bounded preserving-plan vectored prefix copy) as **proven-material**; WP3, WP4, and WP6 are **low-cost-cleanup** items that may be implemented but must be reported as no-ops if no measured gain; WP2 and WP5 are **not-material** / **inconclusive** and must NOT drive production work. All implementation thresholds are frozen in the M042 closure. |
| M044 | `044-datagram-steady-state-synchronization-and-association-scale-optimization.md` | **ready** | M042 (closed) | Datagram steady-state / scale optimization, evidence-gated by M042 target matrix. M042 classifies WP2 (remove whole-`DatagramProxySpec` clone from steady-state ingress) and WP3 (replace association-map async mutex with a short-critical-section design) as **proven-material**; WP4, WP5, and WP6 are **low-cost-cleanup** items that may be implemented but require an explicit no-op disposition if no measured gain; WP7 (deadline-driven reaper) is **not-material** and must NOT be implemented. All implementation thresholds are frozen in the M042 closure. |
