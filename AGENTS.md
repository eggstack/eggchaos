...
M000–M041 are closed. M019 (`ca527db`) remains the final pre-tag authority; M041 (`724b967`, hosted run `36219464594` 13/13) is the latest corrective authority. Performance tranche: M042, M043 (closed at `67ce4ab`), M044 (closed at `7a6dbb8`), and M045 (closed on `a27a67a`, hosted run `36255454409` 13/13) are complete with no automatic successor. M042 remains the frozen measurement authority — do not retune its thresholds from post-optimization output. M042 is measurement-only and must freeze target classifications/thresholds before production optimization. Do not rewrite `plans/closure/` / `plans/archive/` history. Do not start a generic C ABI, Node native addon, JNI, P/Invoke, cgo, UniFFI, or WASM without a separate ADR after demonstrated multi-consumer demand; do not add `eggreplay-*`/`eggprobe-*` production dependencies (downstream adoption only).
