...
M024 is now ready and is the only active handoff. It is semantics-preserving: first add a topology-matched bare UDP relay benchmark and separate sequential RTT from windowed throughput, then optimize only profiled hot paths, replace full-queue scan/sort scheduling, remove association-registry lock ownership across UDP setup awaits, and split the datagram runtime internally. Do not change ADR 003, datagram golden traces, native contracts, or per-client upstream socket ownership to satisfy the performance pass.
