...
M000–M041 are closed. M019 (`ca527db`) remains the final pre-tag authority; M041 (`724b967`, hosted run `36219464594` 13/13) is the latest ADR 007 corrective authority. Performance implementation tranche M042–M045 is closed; M046 is `ready` as the sole provenance/closure-lineage corrective. M046 must not change production/benchmark source or retune M042 thresholds: distinguish production revision, execution base HEAD, evidence commit, artifact refresh, and hosted SHA; preserve historical raw measurements additively instead of rewriting their embedded metadata. Do not rewrite `plans/archive/` history. Do not start a generic C ABI, Node native addon, JNI, P/Invoke, cgo, UniFFI, or WASM without a separate ADR after demonstrated multi-consumer demand; do not add `eggreplay-*`/`eggprobe-*` production dependencies (downstream adoption only).
